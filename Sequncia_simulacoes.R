...
distNormalSimulacao <- rnorm(300) # usa a função rnorm para criar uma distribuição normal, indicando o total de casos

summary(distNormalSimulacao) # sumário da distNormalSimulacao

# distribuição binomial simulada
distBinomialSimulacao <- rbinom(300, 1, 0.7) # rbinom para criar distribuição binominal, indicando casos, tamanho e probabilidade

# repetições

classeSimulacao <- c(rep("slim", length(distBinomialSimulacao)/2), rep("pluz size", length(distBinomialSimulacao)/2)
